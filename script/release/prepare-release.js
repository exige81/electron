...
async function createRelease (branchToTarget, isBeta) {
  ...
  if (isBeta) {
    ...
    // FIXME KEELEY: Need to verify that newVersion is printing what I think it's printing
    } else if (newVersion.indexOf('alpha') > 0) {
      releaseBody = 'Note: This is a alpha release.  Please file new issues ' +
        'for any bugs you find in it.\n \n This release is published to npm ' +
        'under the electron-alpha package and can be installed via `npm install electron-alpha`, ' +
        `or \`npm install electron-alpha@${newVersion.substr(1)}\`.\n \n ${releaseNotes.text}`;
    } else {
...
async function changesToRelease () {
  // FIXME KEELEY: Verify this Regex change is correct
